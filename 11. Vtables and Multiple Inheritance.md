...
# Additional Stuff (Spring 2022)
## Template for functions

```cpp
template<typenmae T> min(T x, T y) {
	return x < y ? x : y;
}

int f() {
	int x = 1, y = 2;
	int z = min(x, y); // T is going to be a int
	// OR you can even do:
	auto z = min(x, y);
	// OR you can do:
	z = min<int>(x, y);
	// OR
	auto f = min(3.0, 1.0); // f is double
}
```

This type induction is for functions only.

## Using template for abstract Iterator for_each
```cpp
template <typename Iter, typename Func> void for_each(Iter start, Iter end, Func f) {
	// exactly as before
}
...
- ``for_each``
Exactly what we have just implemented.

- ``find``
Return the Iterator to the first element from first to last (not including last) with matching val.
...
- ``count``
Same as find, but return number of occurences of val.

- ``copy``
Copies one container range ``[first, last)`` to another container.  
Note: It does NOT allocate new memeory.  
```cpp
...
- ``transform_``
applies a function to all values in a container and returns the results.  
```cpp
template<typename InIter, typename OutIterm typename Func>
...
transform(v.begin(), v.end(), w.begin(), IncreasePlus()); // w = {0,1,2,3,4}
```
